...
def slice_by_line_col(text: str, sl: int, sc: int, el: int, ec: int) -> str:
    ...
    if sl < 0 or el < 0:
        return text
    sl = max(1, min(sl, n)); el = max(1, min(el, n))
    sc = max(0, sc); ec = max(0, ec)
    if (el, ec) < (sl, sc): return ""
    parts = []
    for i in range(sl, el + 1):
        line = lines[i - 1]
        if i == sl and i == el: parts.append(line[sc:ec])
        elif i == sl: parts.append(line[sc:])
        elif i == el: parts.append(line[:ec])
...
def try_send_to_running_instance(payload: bytes) -> bool:
    ...
    if not sock.waitForConnected(150): return False
    sock.write(payload)
    sock.flush()
...
def main():
    ...
    if not code and data:
        code = slice_by_line_col(
            data, args.sel_start_line, args.sel_start_col,
            args.sel_end_line, args.sel_end_col
        ) or data
    if not code.strip():
        print("No selection provided.", file=sys.stderr); sys.exit(1)
